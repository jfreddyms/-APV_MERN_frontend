...
const Registrar = () => {
  ...
  const [ alerta, setAlerta ] = useState({});
   ...
   const handleSubmit = async e => {
        ...
        if ([nombre, email, password, confirPassword ].includes('')) {
        ...
        try {
            ...
            setAlerta({ msg: `User created successfully, Check the confirmation email at ${email} `, error: false});
  ...
  return (
   <>
        <div>
            <h1 className="text-indigo-600  font-black text-6xl ">Create an Account and  Manage {""}<span className="text-black">your Patients</span></h1>
        </div>
           ...
           {msg && 
            ...
            <form onSubmit={handleSubmit}>
                <div className="my-5">
                    ...
                    <input type="text" placeholder="Name" className="border w-full p-3 mt-3 bg-gray-50 rounded-xl" 
                           value={nombre}
                ...
                <input type="submit" value="SIGN IN" className="bg-indigo-700 w-full py-3 px-10 rounded-xl text-white uppercase font-bold mt-5 hover:bg-indigo-800 md:w-auto"  />
            </form>
            ...
            <nav className="mt-10 lg:flex lg:justify-between">
                ...
                <Link className="block text-center my-5 text-gray-500" to="/olvide-password">Forgot Password?</Link>
            </nav>
        </div>
   </>
  )
